# wallace <- function(gene_data, epi_data){
#   
#   epi <- epi_data
#   cgf <- gene_data
#   
...
#   
#   cut_cgf <- cutree(w_cgf, h = .5*wmax_cgf)
#   cut_epi <- cutree(w_epi, h = .5*wmax_epi)
#   
#   wallace_list <- adj_wallace(cut_cgf, cut_epi)
#   return(wallace_list)
# }
...
#            sub = paste("Entanglement: ", round(entanglement(dendo[[1]], dendo[[2]]), 2)),
           sub = paste("AWC Gen vs Epi:", round(wallace_list$Adjusted_Wallace_A_vs_B, 2),
           cex_sub = 0.9)
